...
def main():
    if len(sys.argv) < 4:
        print("Usage: python import_digivice_friend_table.py Digivice.bin digivice_friend_table.csv Digivice.bin")
        return
    ...
    for i, row in enumerate(rows):
        ...
        for name, value in [
            ...
            ("attack_shot_sprite_index", attack_sprite),
            ("attack_shot_sound_id", attack_sound),
            ("unknown", unknown),
        ]:
            if not 0 <= value <= 0xFFFF:
                raise RuntimeError(f"Row {i} {name} out of uint16 range: {value}")
