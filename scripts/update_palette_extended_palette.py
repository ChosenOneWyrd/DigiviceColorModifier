...
For each PNG:
  ...
  - BANK      -> palette bank to write (0..31). If --set-sprite-bank is used,
...
def collect_bank_usage(images, sprites, target_pal_start):
    """Return set of banks (0..31) referenced by any image sharing palette_start_index."""
    used: Set[int] = set()
...
def update_one(data, pkg_off, offs, images, sprites, image_index, subimage, png_path, target_bank, alpha_mode, set_sprite_bank, dry_run):
    ...
    if isinstance(target_bank, str) and target_bank == "auto":
        ...
        candidates = [b for b in range(32) if b not in used]
        bank = candidates[0] if candidates else 31
    else:
        bank = int(target_bank)
        if not (0 <= bank <= 31):
            raise SystemExit(f"Bank {bank} out of range 0..31")

    inverted = (alpha_mode == "inverted")

    # ---------- Auto-reduce PNG to device-required color count ----------
...
# ---------- batch driver ----------
# Filename regex allows any bank number
