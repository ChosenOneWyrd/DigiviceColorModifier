...
def main():
    if len(sys.argv) < 4:
        print("Usage: python import_digivice_map_areas.py Digivice.bin digivice_map_areas.csv Digivice_modified.bin")
        return
    ...
    if len(csv_rows) != NUM_RECORDS:
        raise ValueError(f"Expected {NUM_RECORDS} rows but found {len(csv_rows)}")
    for i, row in enumerate(csv_rows):
        body = row[3:]
        if len(body) != 25:
            raise ValueError(f"Row {i}: expected body length 25")
        records[i][:25] = body
    for i in range(NUM_RECORDS - 1):
        next_row = csv_rows[i + 1]
        records[i][25] = next_row[0]
        records[i][26] = next_row[1]
        records[i][27] = next_row[2]
